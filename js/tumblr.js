	var numPostsToDisplay = 5;
	var postCount = 0;
	var imagePath = 'http://chris-tran.com/images/tumblr/';
	$(document).ready(function() {	
/*
	This provides more error handling in case of timeout issues.
 */
				$.ajax({
                type: "GET",
                url: "http://blog.whoiscinch.com/read/json?callback=?",
                dataType:"json",
		...
		$.getJSON("http://blog.whoiscinch.com/api/read/json?callback=?", 
  		function(data) {
    		$.each(data.posts.slice(0, 10), function(i,post){
					if (postCount >= numPostsToDisplay) {
						return;
		...
		$.ajax({
		    type: "GET",
		    url: "http://blog.whoiscinch.com/api/read/",
		    dataType: "xml",
		    success: parseTumblrXml
		  });		    
    ...
    {		    	
    	...
    	case "quote":
    	{
    		...
    		$("#tumblrFeed").append("<div class='body'>" + 
    			"<div class='quote'>" + post["quote-text"] + "</div>" +
    	...
    	case "conversation":
    	{
    		...
    		$("#tumblrFeed").append("<a class='title' href='" + post.url + "' target='_blank'>" + post["conversation-title"] + "</a>");
				...
				for(var i = 0; i < post["conversation"].length; i++) {
					...
					html += "<div class='convtext'>" + conv.phrase + "</div>";
				}
